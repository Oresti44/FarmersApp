...
function TasksList({
  ...
  function toggleSelection(taskId) {
    if (selectedIds.includes(taskId)) {
      setSelectedIds(selectedIds.filter((value) => value !== taskId))
      return
    }
  ...
  return (
    <section className="rounded-[32px] border border-white/80 bg-white/86 shadow-[0_24px_80px_rgba(82,97,69,0.1)]">
      {actingRole === 'manager' && selectedIds.length ? (
        <div className="flex flex-wrap items-center gap-3 border-b border-stone-200 px-6 py-4">
          <span className="text-sm font-semibold text-stone-700">{selectedIds.length} selected</span>
          <button type="button" onClick={onBulkStatus} className="rounded-full bg-stone-100 px-4 py-2 text-sm">
            Change status
          </button>
          <button type="button" onClick={onBulkMove} className="rounded-full bg-stone-100 px-4 py-2 text-sm">
            Move date/time
          </button>
          <button type="button" onClick={onBulkPriority} className="rounded-full bg-stone-100 px-4 py-2 text-sm">
            Change priority
          </button>
          <button
            type="button"
              ...
              {actingRole === 'manager' ? (
                <th className="px-4 py-4">
                  <input
                    type="checkbox"
                    checked={allSelected}
                    onChange={() => setSelectedIds(allSelected ? [] : tasks.map((task) => task.id))}
                  />
                </th>
              ) : null}
