function HarvestHistoryTable({ entries, onAction }) {
  return (
        ...
        <table className="min-w-full border-separate border-spacing-0">
          <thead className="bg-[#fbfaf6] text-left text-xs uppercase tracking-[0.18em] text-stone-500">
            <tr>
              ...
              <th className="px-4 py-4">Harvested at</th>
          ...
          <tbody>
            {entries.map((entry) => (
              <tr key={entry.id} className="text-sm text-stone-700">
                ...
                <td className="px-4 py-4 align-top">{new Date(entry.harvested_at).toLocaleString()}</td>
                <td className="px-4 py-4 align-top">
                  {entry.quantity} {entry.quantity_unit}
                </td>
                <td className="px-4 py-4 align-top">{entry.quality_grade || '-'}</td>
                <td className="px-4 py-4 align-top">{entry.notes || '-'}</td>
...
export default HarvestHistoryTable
