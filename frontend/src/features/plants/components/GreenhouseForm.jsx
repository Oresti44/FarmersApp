    ...
    <DrawerShell open={open} onClose={onClose} title={greenhouse?.id ? 'Edit greenhouse' : 'Add greenhouse'}>
      <form onSubmit={submit} className="grid gap-5">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button type="submit" className="rounded-full bg-stone-950 px-5 py-2 text-sm font-semibold text-white">{greenhouse?.id ? 'Save greenhouse' : 'Create greenhouse'}</button>
        </div>
      </form>
    </DrawerShell>
  )
